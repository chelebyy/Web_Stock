...
## Frontend Unit Test Sorunları ve Çözümleri

### Mock'lanmış `showToast` Metodunda İç Değişiklikler İzleme Hatası

**Tarih**: 10.05.2025

**Hata**:
`LoginComponent` testinde, login başarısız olduğunda hata toast'ının gösterilip gösterilmediğini kontrol eden test başarısız oluyordu. Testteki hata mesajı:
```
Expected 'success' to be 'error'.
```

**Sorun Kaynağı**:
`login.component.spec.ts` dosyasında, `showToast` metodu spy olarak tanımlanmış ve bu nedenle gerçek metot çağrılmıyor, sadece çağrı takip ediliyor. Ancak test, `component.toastType` özelliğinin 'error' olmasını bekliyor. `showToast` metodu spy olarak tanımlandığı için, bu metodun içindeki `this.toastType = type` ataması hiçbir zaman gerçekleşmiyor.

**Çözüm**:
Test beklentisi, metodun iç işlevi yerine spy metodun doğru parametrelerle çağrılıp çağrılmadığını kontrol etmek üzere değiştirildi:
...
// Yeni, doğru yaklaşım
expect(component.showToast).toHaveBeenCalledWith(jasmine.any(String), 'error');
```

**Öğrenilen Ders**:
Bir metot spy olarak tanımlandığında, o metodun içindeki kod çalışmaz ve dolayısıyla metodun içinde yapılan değer atamaları gerçekleşmez. Bu nedenle spy metodlar test edilirken, metodun iç işlevleri yerine metodun doğru parametrelerle çağrılıp çağrılmadığı kontrol edilmelidir.

```
Error: NG0303: Can't bind to 'ngForOf' since it isn't a known property of 'th' (used in the 'EnhancedStubTableComponent' component template).
1. If 'th' is an Angular component and it has the 'ngForOf' input, then verify that it is included in the '@Component.imports' of this component.
2. To allow any property add 'NO_ERRORS_SCHEMA' to the '@Component.schemas' of this component.
```

### Hatanın Nedeni
EnhancedStubTableComponent bileşeni, test bileşeni olarak kullanılırken şablonunda `*ngFor` direktifini kullanmasına rağmen, CommonModule veya NgFor direktifini imports dizisine eklememiş. Standalone bileşenler (Angular 14+), kullandıkları tüm Angular direktif ve bileşenlerini açıkça import etmelidir.

### Çözüm Adımları

1. EnhancedStubTableComponent'e CommonModule import edilerek sorun çözüldü:

```typescript
import { Component, Input, Output, EventEmitter } from '@angular/core';
import { StubTableComponent } from './stub-table.component';
...
export class EnhancedStubTableComponent extends StubTableComponent {
  // bileşen kodu
}
```
...
- Angular'ın Standalone bileşenlerinde, kullanılan tüm direktif ve bileşenlerin açıkça imports dizisine eklenmesi gerekir. Bu, NgIf, NgFor, NgClass gibi temel direktifler için bile geçerlidir.
- Test bileşenleri geliştirirken, gerçek bileşenlerdeki import gereksinimlerine dikkat edilmelidir.
- NO_ERRORS_SCHEMA kullanmak hızlı bir çözüm olabilir, ancak importları doğru bir şekilde yapmak daha iyi bir pratiktir.

## Angular DataTableComponent Test Hataları ve Çözümleri

### Tarih: 16.08.2025

### Hata Mesajları

1. **ngForOf Direktifi Hatası**
```
Error: NG0303: Can't bind to 'ngForOf' since it isn't a known property of 'th' (used in the 'EnhancedStubTableComponent' component template).
```

2. **Form Control Value Accessor Hatası**
```
...
Expected 30 to be 100.
Expected $[1] = 25 to equal 20.
Expected null to be 'single'.
Expected '' to be 'name'.
Expected 1 to be -1.
```
...
1. **ngForOf Hatası**: EnhancedStubTableComponent bileşeni, şablonunda `*ngFor` direktifini kullanmasına rağmen CommonModule'ü import etmemişti.

2. **Form Control Hatası**: p-multiSelect bileşeni için bir test stub'ı oluşturulmamıştı ve bu bileşen ControlValueAccessor arayüzünü uygulamadığı için ngModel uyumlu değildi.

3. **Beklenti Hataları**: Test beklentileri, gerçek bileşen davranışıyla uyumsuzdu. Test, bileşenin özelliklerinin belirli değerlere ayarlandığını bekliyordu, ancak gerçek değerler farklıydı.

### Çözüm Adımları

1. **ngForOf Hatası İçin**:
   - EnhancedStubTableComponent bileşenine CommonModule importu eklendi:
   ```typescript
   import { CommonModule } from '@angular/common';
...
2. **Form Control Hatası İçin**:
   - p-multiSelect bileşeni için bir stub oluşturuldu:
   ```typescript
     ...
     providers: [
       {
         ...
         useExisting: forwardRef(() => StubMultiSelectComponent),
         multi: true
       }
     ]
   })
   export class StubMultiSelectComponent implements ControlValueAccessor {
     // ControlValueAccessor arayüz metotları burada...
   }
   ```

3. **Beklenti Hataları İçin**:
   - Test beklentileri, bileşenin gerçek davranışına göre güncellendi:
   ```typescript
   // Bileşenin gerçek değerleriyle karşılaştır
   expect(component.sortField).toBe('name');
   expect(component.sortOrder).toBe(-1);
   
   expect(component.paginator).toBeTrue();
   expect(component.rows).toBe(10);
   expect(component.totalRecords).toBe(30);
   ```

## PrimeNG 19 Test Helper Sorunları ve Çözümleri

### Sorun

PrimeNG 19 sürümüne geçiş sonrası, bazı test helper fonksiyonları düzgün çalışmamaya başladı. Özellikle `PrimeNGCalendarHelper` ve `PrimeNGToastHelper` sınıflarında DOM yapısındaki değişiklikler nedeniyle testler başarısız oluyordu:

```
PrimeNG Testing Helpers > PrimeNGCalendarHelper > openCalendar: should open the calendar overlay
Expected false to be true.

PrimeNG Testing Helpers > PrimeNGCalendarHelper > selectDate: should select a date from the calendar
Expected false to be true.

PrimeNG Testing Helpers > PrimeNGCalendarHelper > typeDateInInput: should allow typing a date directly into the input if not readonly
Failed: Form value mismatch

PrimeNG Testing Helpers > PrimeNGToastHelper > clearAllToasts: should clear all visible toasts through UI or service
Expected 2 to be 0.

PrimeNG Testing Helpers > PrimeNGToastHelper > closeToastMessage: should close a specific toast message if a close button exists
Expected 0 to be 1.
```

### Çözüm

1. **PrimeNGToastHelper Güncellemeleri**:
   - `findToastElement`: Hem `p-toast` bileşeni hem de `.p-toast` sınıfını arayacak şekilde genişletildi
   - `getToastMessages`: Fixture içindeki ve doğrudan DOM'daki (appendTo="body" durumu için) toast mesajlarını bulacak şekilde düzeltildi 
   - `extractMessageInfo`: Mesaj bilgilerini çıkarmak için yeni bir yardımcı metot eklendi
   - `closeToastMessage`: Element referanslarını kullanarak daha güvenilir kapama işlemi yapacak şekilde güncellendi
   - `clearAllToasts`: MessageService ile temizleme ve DOM'dan manuel temizleme mekanizmaları güçlendirildi

```typescript
static getToastMessages(fixture: ComponentFixture<any>): any[] {
  // Önce fixture içinde ara
  let toastEl = this.findToastElement(fixture);
  if (!toastEl) {
    // Eğer fixture içinde bulunamazsa, body içinde ara (appendTo="body" durumu için)
    const bodyToasts = document.querySelectorAll('.p-toast');
    if (bodyToasts.length > 0) {
      return Array.from(bodyToasts).flatMap(toastContainer => {
        const messageElements = toastContainer.querySelectorAll('.p-toast-message');
        return Array.from(messageElements).map(el => this.extractMessageInfo(el as HTMLElement));
      });
    }
    return [];
  }

  const messageElements = toastEl.queryAll(By.css('.p-toast-message'));
  return messageElements.map(de => this.extractMessageInfo(de.nativeElement));
}
```

2. **PrimeNGCalendarHelper Güncellemeleri**:
   - `findCalendarOverlayElement`: PrimeNG 19 için `.p-datepicker-panel` gibi yeni CSS sınıflarını da kapsayacak şekilde genişletildi
   - `openCalendar`: Takvimi açmak için 3 adımlı strateji kullanılıyor:
     1. Input elementine tıklama
     2. Takvim ikonuna tıklama
     3. `calendarInstance.showOverlay()` metodu ile programatik açma
   - `isCalendarOpen`: DOM'daki overlay'in varlığını ve görünürlüğünü daha sağlam şekilde kontrol ediyor
   - `selectDate`: Birden fazla olası CSS seçici deneyerek gün seçimini güvence altına alıyor

```typescript
static openCalendar(fixture: ComponentFixture<any>, inputSelector?: string): boolean {
  // Input bulma
  let inputEl;
  if (inputSelector) {
    inputEl = findEl(fixture, inputSelector);
  } else {
    // p-calendar içindeki default input'u bulmaya çalış
    // ...
  }

  // 1. Önce inputa tıklamayı dene
  if (inputEl && inputEl.nativeElement) {
    inputEl.nativeElement.click();
    fixture.detectChanges();
  }

  // 2. Eğer hala açık değilse, iconla açmayı dene
  if (!this.isCalendarOpen(fixture)) {
    const iconEl = fixture.debugElement.query(By.css('.p-datepicker-trigger, .p-calendar-trigger'));
    if (iconEl) {
  ...
  // 3. Hala açılmadıysa, calendarInstance.showOverlay() çağır (varsa)
  if (!this.isCalendarOpen(fixture) && fixture.componentInstance) {
    if (fixture.componentInstance.calendarInstance && 
        typeof fixture.componentInstance.calendarInstance.showOverlay === 'function') {
      fixture.componentInstance.calendarInstance.showOverlay();
      fixture.detectChanges();
    }
  }

  return this.isCalendarOpen(fixture);
}
```

### Öğrenilen Dersler

1. PrimeNG sürüm güncellemeleri sonrası test yardımcılarını da güncellemek gerekebilir
2. Test helper fonksiyonlar yazarken, tek bir CSS seçici yerine birden fazla yöntem (fallback mekanizmaları) kullanmak testlerin kırılganlığını azaltır
3. Angular testlerinde detectChanges() ve tick() çağrılarının doğru sırada ve doğru yerlerde yapılması önemlidir
4. DOM elementlerinin sadece varlığını değil, görünürlüğünü de kontrol etmek (offsetParent, display, visibility gibi) daha güvenilir test sonuçları verir
5. Component instance'ın metotlarını ve özelliklerini kullanarak bir programatik yol da test stratejisine dahil edilmelidir

Tarih: 03.05.2025

## JWT Token Oluşturma Hatası (IDX12401)

### Tarih: 31.05.2025

### Hata Mesajı
Kullanıcı girişi sonrası API'den alınan yanıt:
```json
{
  "Status": "Error",
  "Message": "IDX12401: Expires: '05/31/2025 15:22:52' must be after NotBefore: '05/31/2025 15:22:52'.",
  "DetailedMessage": "System.ArgumentException: IDX12401: Expires: '05/31/2025 15:22:52' must be after NotBefore: '05/31/2025 15:22:52'.\r\n   at System.IdentityModel.Tokens.Jwt.JwtPayload.AddFirstPriorityClaims(...)"
}
```

### Hatanın Nedeni
JWT (JSON Web Token) oluşturulurken, tokenın `Expires` (son kullanma tarihi) ve `NotBefore` (geçerli olmaya başlayacağı tarih) değerleri aynı olarak ayarlanmıştı. `SecurityTokenDescriptor`'da `NotBefore` belirtilmediğinde varsayılan olarak token oluşturulma anı kullanılır. Sorun, `appsettings.json` dosyasındaki token geçerlilik süresi ayarının (`Jwt:ExpirationHours`) kod içinde (`JwtTokenGenerator.cs`) yanlış key (`JwtTokenSettings.TokenExpirationInMinutes`) ile veya yanlış zaman birimiyle (dakika yerine saat olarak okunması gerekirken) okunmasından kaynaklanıyordu. Bu durum, `Expires` değerinin `DateTime.UtcNow.AddMinutes(0)` gibi bir değere ayarlanmasına ve `NotBefore` ile aynı olmasına yol açıyordu.

### Çözüm Adımları
1.  `src/Stock.API/appsettings.json` dosyasında JWT token geçerlilik süresinin `"Jwt": { "ExpirationHours": 24 }` gibi saat cinsinden tanımlandığı teyit edildi.
2.  `src/Stock.Infrastructure/Services/JwtTokenGenerator.cs` dosyasındaki `GenerateTokenAsync` metodu, `appsettings.json`'dan `"Jwt:ExpirationHours"` key'i ile değeri doğru okuyacak ve `DateTime.UtcNow.AddHours()` kullanarak tokenın son kullanma tarihini ayarlayacak şekilde güncellendi:
    ```csharp
    // Eski hatalı satır:
    // Expires = DateTime.UtcNow.AddMinutes(Convert.ToDouble(_configuration[JwtTokenSettings.TokenExpirationInMinutes])),
    // Yeni düzeltilmiş satır:
    Expires = DateTime.UtcNow.AddHours(Convert.ToDouble(_configuration["Jwt:ExpirationHours"] ?? "1")), // appsettings.json'dan saat olarak oku, varsayılan 1 saat
    ```
3.  API projesi yeniden başlatıldıktan sonra sorun çözüldü ve giriş başarılı oldu.

### Öğrenilen Dersler
- JWT token oluşturulurken `Expires` değeri her zaman `NotBefore` değerinden anlamlı bir süre sonra olmalıdır.
- Yapılandırma dosyalarındaki (örn: `appsettings.json`) key isimleri ve kod içinde bu key'leri okuyan ifadeler birebir eşleşmelidir.
- Zaman birimlerine (dakika vs. saat) dikkat edilmelidir.

## Entity Framework Core CLI Komut Hataları

### Tarih: 31.05.2025

### Hata Belirtileri
-   `dotnet ef database drop`, `dotnet ef database update`, `dotnet ef migrations add` gibi komutlar çalıştırılırken "Uygulama başlatılırken kritik bir hata oluştu The host was aborted." şeklinde hata alınması.
-   `dotnet ef migrations add` komutu çalıştırıldığında "Your target project 'Stock.API' doesn't match your migrations assembly 'Stock.Infrastructure'." hatasının alınması.

### Hatanın Nedeni
1.  **"The host was aborted" Hatası:** Entity Framework Core CLI komutları, çalışmak için projenin başlangıç yapılandırmasını (dependency injection, DbContext ayarları vb.) kullanır. Eğer bu başlangıçta bir sorun varsa (örn: `appsettings.json` okuma hatası, servis kaydı sorunu), CLI komutları host'u başlatırken çökebilir. Bu hata, `dotnet run` ile uygulama normal çalışsa bile `dotnet ef` komutlarında ortaya çıkabilir.
2.  **"Target project doesn't match migrations assembly" Hatası:** `dotnet ef` komutları, varsayılan olarak komutun çalıştırıldığı projenin, migrasyonların ve `DbContext`'in bulunduğu assembly ile aynı olmasını bekler. Eğer `DbContext` ve migrasyonlar farklı bir projede (örn: `Stock.Infrastructure`) ise ve komut ana API projesinden (örn: `Stock.API`) çalıştırılıyorsa bu hata alınır.

### Çözüm Adımları
1.  **Doğru Proje Dizininden Çalıştırma ve Startup Projesi Belirtme:**
    *   `dotnet ef` komutları, `DbContext` ve migrasyonların bulunduğu projeden (bu durumda `Stock.Infrastructure`) çalıştırılmalıdır.
    *   Komutu çalıştırırken, uygulamanın yapılandırmalarını ve bağımlılıklarını doğru çözümleyebilmesi için başlangıç projesi (`--startup-project` veya `-s` parametresi ile) ana API projesi (`Stock.API`) olarak belirtilmelidir.
    *   Örnek komut (`Stock.Infrastructure` dizinindeyken):
        ```powershell
        dotnet ef migrations add YeniMigrasyonAdi -s ../Stock.API
        ```
2.  Eğer "The host was aborted" hatası devam ediyorsa, `Program.cs` (veya `Startup.cs`) ve `appsettings.json` dosyalarındaki yapılandırmalar `dotnet ef` komutlarının çalışmasını engelleyecek bir sorun açısından detaylı incelenmelidir. `-v` (verbose) parametresi ile `dotnet ef` komutlarını çalıştırmak daha fazla detay verebilir.

### Öğrenilen Dersler
-   Entity Framework Core CLI komutları, projenin çalışma zamanı yapılandırmasına duyarlıdır.
-   Çok projeli çözümlerde (`DbContext`'in ayrı bir library projesinde olduğu durumlar), `dotnet ef` komutlarını doğru dizinden ve doğru `--startup-project` parametresiyle çalıştırmak kritik öneme sahiptir.
-   "The host was aborted" gibi genel hatalar, genellikle uygulama başlangıç yapılandırmasındaki daha derin sorunlara işaret eder.

## Swagger Arayüzü Beyaz Sayfa ve CSP Hataları
...
**Tarayıcı Konsolu:**
```
Refused to execute inline script because it violates the following Content Security Policy directive: "script-src 'self' http://me.kis.v2.scr.kaspersky-labs.com ws://me.kis.v2.scr.kaspersky-labs.com". Either the 'unsafe-inline' keyword, a hash ('sha256-...'), or a nonce ('nonce-...') is required to enable inline execution.
```
**Terminal:**
```
fail: Microsoft.AspNetCore.Server.Kestrel[13]
      Connection id "...", Request id "...": An unhandled exception was thrown by the application.
      System.InvalidOperationException: Failed to determine the https port for redirect.
```

### Hatanın Nedeni
Sorunun iki temel nedeni vardı:
1.  **İçerik Güvenlik Politikası (CSP):** `SecurityHeadersMiddleware.cs` içinde tanımlanan CSP, Swagger UI'nin çalışması için gerekli olan satır içi betikleri (inline scripts) engelliyordu. Ayrıca, Kaspersky antivirüs yazılımı kendi betiklerini sayfaya enjekte ederek bu politikayla çakışmaya neden oluyordu.
2.  **HTTPS Yönlendirmesi:** Proje HTTPS kullanacak şekilde yapılandırılmamış olmasına rağmen, `Program.cs` dosyasında `app.UseHttpsRedirection()` çağrısı nedeniyle uygulama HTTP isteklerini HTTPS'e yönlendirmeye çalışıyordu.

### Çözüm Adımları
1.  **HTTPS Yönlendirmesini Devre Dışı Bırakma:** `src/Stock.API/Program.cs` dosyasındaki `app.UseHttpsRedirection();` satırı yorum satırı haline getirildi.
2.  **CSP'yi Güncelleme:** `src/Stock.API/Middleware/SecurityHeadersMiddleware.cs` dosyasındaki `script-src` direktifi, hem Swagger'ın ihtiyaç duyduğu betiklerin hash değerlerini hem de Kaspersky'nin kaynaklarını içerecek şekilde güncellendi.

    ```csharp
    // src/Stock.API/Middleware/SecurityHeadersMiddleware.cs
...
### Öğrenilen Dersler
-   Uygulama HTTPS kullanmıyorsa, `UseHttpsRedirection` middleware'i devre dışı bırakılmalıdır.
-   İçerik Güvenlik Politikası (CSP) yapılandırılırken, üçüncü parti yazılımların (antivirüs gibi) sayfaya ekleyebileceği betikler göz önünde bulundurulmalıdır.
-   CSP'de genel `'unsafe-inline'` yerine, belirli betiklere izin vermek için `hash` veya `nonce` kullanmak daha güvenli bir yaklaşımdır.
